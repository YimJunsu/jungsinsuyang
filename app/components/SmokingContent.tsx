'use client';

import { useState, useEffect, useRef, useCallback } from 'react';

/* ─── Cigarette geometry (px) ─────────────────────────
 *  Top → Bottom:
 *    0          : ash top (grows downward as burns)
 *    ashH       : ember (always EMBER_H tall)
 *    ashH+EMBER : remaining tobacco body (shrinks)
 *    BAND_TOP   : gold band  ← fixed position
 *    FILTER_TOP : cork filter ← fixed position
 *    CIGA_TOTAL : bottom of cigarette
 * ─────────────────────────────────────────────────── */
const CIGA_W     = 26;
const BODY_H     = 190;
const EMBER_H    = 13;
const BAND_H     = 6;
const FILTER_H   = 84;
const CIGA_TOTAL = BODY_H + EMBER_H + BAND_H + FILTER_H; // 293

// These are FIXED regardless of burn progress:
const BAND_TOP   = BODY_H + EMBER_H;             // 203
const FILTER_TOP = BODY_H + EMBER_H + BAND_H;    // 209

const TOTAL_BURN_SECONDS = 120; // 2 minutes

/* ─── Smoke particle ──────────────────────────────── */
interface Particle {
  x: number; y: number;
  vx: number; vy: number;
  size: number;       // current radius
  maxSize: number;    // target radius at end of life
  life: number;       // 1 = fresh → 0 = dead
  maxLife: number;    // total lifetime in seconds
  baseOpacity: number;
  turbPhase: number;  // phase offset for sine turbulence
  turbAmp: number;    // horizontal turbulence amplitude
}

/* ══════════════════════════════════════════════════════
 *  Main Component
 * ══════════════════════════════════════════════════════ */
export default function SmokingContent() {
  const [burnProgress, setBurnProgress] = useState(0);
  const [isActive, setIsActive]         = useState(false);
  const [isDone, setIsDone]             = useState(false);

  /* Refs for cross-closure access inside rAF loops */
  const burnedRef      = useRef(0);
  const bpRef          = useRef(0);          // burnProgress mirror for canvas loop
  const isActiveRef    = useRef(false);
  const isDoneRef      = useRef(false);
  const lastBurnTsRef  = useRef(0);
  const burnRafRef     = useRef<number>(0);

  /* Web Audio – cigarette crackle */
  const audioCtxRef    = useRef<AudioContext | null>(null);
  const crackleGainRef = useRef<GainNode | null>(null);
  const audioReadyRef  = useRef(false);

  /* Canvas / particle system */
  const canvasRef      = useRef<HTMLCanvasElement>(null);
  const containerRef   = useRef<HTMLDivElement>(null);
  const particlesRef   = useRef<Particle[]>([]);
  const smokeRafRef    = useRef<number>(0);
  const lastSmokeTsRef = useRef(0);
  const spawnAccRef    = useRef(0);
  const hazeRef        = useRef(0); // accumulated room fog [0-1]

  /* ── Burn tick (rAF, only while active) ─────────── */
  const burnTick = useCallback((ts: number) => {
    if (!isActiveRef.current) return;
    if (lastBurnTsRef.current === 0) lastBurnTsRef.current = ts;
    const dt = Math.min((ts - lastBurnTsRef.current) / 1000, 0.08);
    lastBurnTsRef.current = ts;
    burnedRef.current = Math.min(burnedRef.current + dt, TOTAL_BURN_SECONDS);
    const p = burnedRef.current / TOTAL_BURN_SECONDS;
    bpRef.current = p;
    setBurnProgress(p);
    if (p >= 1) {
      isDoneRef.current = true;
      isActiveRef.current = false;
      setIsDone(true);
      setIsActive(false);
      return;
    }
    burnRafRef.current = requestAnimationFrame(burnTick);
  }, []);

  /* ── Init cigarette crackle audio ──────────────── */
  const initCrackle = useCallback(() => {
    if (audioReadyRef.current) return;
    audioReadyRef.current = true;

    const ctx = new (window.AudioContext || (window as unknown as { webkitAudioContext: typeof AudioContext }).webkitAudioContext)();
    if (ctx.state === 'suspended') ctx.resume();

    /* White noise buffer (2 s loop) */
    const sr     = ctx.sampleRate;
    const buf    = ctx.createBuffer(1, sr * 2, sr);
    const data   = buf.getChannelData(0);
    for (let i = 0; i < data.length; i++) data[i] = Math.random() * 2 - 1;

    const src = ctx.createBufferSource();
    src.buffer = buf;
    src.loop   = true;

    /* Bandpass → crackle texture */
    const bp = ctx.createBiquadFilter();
    bp.type            = 'bandpass';
    bp.frequency.value = 3200;
    bp.Q.value         = 0.6;

    /* High-shelf boost for sizzle */
    const hs = ctx.createBiquadFilter();
    hs.type            = 'highshelf';
    hs.frequency.value = 6000;
    hs.gain.value      = 8;

    const gain = ctx.createGain();
    gain.gain.value = 0;

    src.connect(bp);
    bp.connect(hs);
    hs.connect(gain);
    gain.connect(ctx.destination);
    src.start();

    audioCtxRef.current    = ctx;
    crackleGainRef.current = gain;
  }, []);

  const startBurning = useCallback(() => {
    if (isDoneRef.current) return;
    isActiveRef.current = true;
    setIsActive(true);
    lastBurnTsRef.current = 0;
    burnRafRef.current = requestAnimationFrame(burnTick);

    initCrackle();
    if (crackleGainRef.current && audioCtxRef.current) {
      crackleGainRef.current.gain.setTargetAtTime(0.18, audioCtxRef.current.currentTime, 0.05);
    }
  }, [burnTick, initCrackle]);

  const stopBurning = useCallback(() => {
    isActiveRef.current = false;
    setIsActive(false);
    cancelAnimationFrame(burnRafRef.current);

    if (crackleGainRef.current && audioCtxRef.current) {
      crackleGainRef.current.gain.setTargetAtTime(0, audioCtxRef.current.currentTime, 0.15);
    }
  }, []);

  const reset = useCallback(() => {
    burnedRef.current     = 0;
    bpRef.current         = 0;
    isActiveRef.current   = false;
    isDoneRef.current     = false;
    hazeRef.current       = 0;
    spawnAccRef.current   = 0;
    particlesRef.current  = [];
    setBurnProgress(0);
    setIsActive(false);
    setIsDone(false);
    cancelAnimationFrame(burnRafRef.current);
    if (crackleGainRef.current && audioCtxRef.current) {
      crackleGainRef.current.gain.setTargetAtTime(0, audioCtxRef.current.currentTime, 0.1);
    }
  }, []);

  /* ── Keyboard ──────────────────────────────────── */
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      if (e.code === 'Space' && !e.repeat) { e.preventDefault(); startBurning(); }
    };
    const onUp = (e: KeyboardEvent) => {
      if (e.code === 'Space') { e.preventDefault(); stopBurning(); }
    };
    window.addEventListener('keydown', onDown);
    window.addEventListener('keyup', onUp);
    return () => {
      window.removeEventListener('keydown', onDown);
      window.removeEventListener('keyup', onUp);
    };
  }, [startBurning, stopBurning]);

  /* ── Canvas smoke loop (always running) ──────── */
  useEffect(() => {
    const canvas    = canvasRef.current;
    const container = containerRef.current;
    if (!canvas || !container) return;

    const resize = () => {
      canvas.width  = container.clientWidth;
      canvas.height = container.clientHeight;
    };
    resize();
    const ro = new ResizeObserver(resize);
    ro.observe(container);

    /* Spawn helper */
    const spawnParticle = (ex: number, ey: number, active: boolean) => {
      particlesRef.current.push({
        x:           ex + (Math.random() - 0.5) * 8,
        y:           ey,
        vx:          (Math.random() - 0.5) * (active ? 22 : 9),
        vy:          -(active ? 42 + Math.random() * 40 : 18 + Math.random() * 18),
        size:        5  + Math.random() * 6,
        maxSize:     active ? 60 + Math.random() * 100 : 32 + Math.random() * 48,
        life:        1,
        maxLife:     active ? 2.8 + Math.random() * 2.4 : 3.2 + Math.random() * 2.2,
        baseOpacity: active ? 0.55 + Math.random() * 0.25 : 0.22 + Math.random() * 0.12,
        turbPhase:   Math.random() * Math.PI * 2,
        turbAmp:     active ? 28 + Math.random() * 36 : 12 + Math.random() * 18,
      });
    };

    const smokeLoop = (ts: number) => {
      /* Delta time */
      if (lastSmokeTsRef.current === 0) lastSmokeTsRef.current = ts;
      const dt = Math.min((ts - lastSmokeTsRef.current) / 1000, 0.05);
      lastSmokeTsRef.current = ts;

      const ctx = canvas.getContext('2d');
      if (!ctx) { smokeRafRef.current = requestAnimationFrame(smokeLoop); return; }

      ctx.clearRect(0, 0, canvas.width, canvas.height);

      /* ── Room haze ────────────────────────────── */
      if (hazeRef.current > 0.015) {
        ctx.fillStyle = `rgba(120, 100, 65, ${hazeRef.current * 0.09})`;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
      }

      /* ── Ember world position ─────────────────── */
      const ashH    = BODY_H * bpRef.current;
      const cigTopY = canvas.height / 2 - CIGA_TOTAL / 2;
      const emberY  = cigTopY + ashH;             // top edge of ember
      const emberX  = canvas.width / 2;

      /* ── Ember ambient glow on background ──────── */
      if (isActiveRef.current && !isDoneRef.current) {
        const gr = ctx.createRadialGradient(
          emberX, emberY + EMBER_H, 0,
          emberX, emberY + EMBER_H, 180
        );
        gr.addColorStop(0,   'rgba(255, 130, 15, 0.11)');
        gr.addColorStop(0.5, 'rgba(255,  70,  0, 0.055)');
        gr.addColorStop(1,   'rgba(255,  30,  0, 0)');
        ctx.fillStyle = gr;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
      }

      /* ── Spawn particles ──────────────────────── */
      if (!isDoneRef.current) {
        const active    = isActiveRef.current;
        const spawnRate = active ? 7 : 0.5; // particles / second
        spawnAccRef.current += spawnRate * dt;
        while (spawnAccRef.current >= 1) {
          spawnParticle(emberX, emberY, active);
          spawnAccRef.current -= 1;
        }
        if (active) hazeRef.current = Math.min(hazeRef.current + 0.00025 * dt, 0.52);
      }

      /* Cull excess particles */
      if (particlesRef.current.length > 110) {
        particlesRef.current = particlesRef.current.slice(-110);
      }
      particlesRef.current = particlesRef.current.filter(p => p.life > 0);

      /* ── Update + draw each particle ─────────── */
      for (const p of particlesRef.current) {
        p.life -= dt / p.maxLife;
        if (p.life <= 0) continue;

        const t  = 1 - p.life; // 0=fresh → 1=about to die

        /* Physics */
        p.x  += (p.vx + Math.sin(t * 5.5 + p.turbPhase) * p.turbAmp * 0.45) * dt;
        p.y  += p.vy * dt;
        p.vy += 28 * dt;      // deceleration (slows upward rise)
        p.vx *= (1 - 0.55 * dt);

        /* Size: grows toward maxSize as particle ages */
        const sz = p.size + (p.maxSize - p.size) * Math.sqrt(t);

        /* Opacity: ramps in then fades, with cutoff near death */
        const op = Math.sin(Math.min(t, 0.75) * Math.PI)
          * p.baseOpacity
          * Math.max(0, 1 - Math.max(0, t - 0.72) * 3.5);

        if (op < 0.008 || sz < 1) continue;

        /* Draw soft radial blob */
        const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, sz);
        g.addColorStop(0,    `rgba(202, 194, 182, ${op * 0.68})`);
        g.addColorStop(0.38, `rgba(178, 170, 158, ${op * 0.42})`);
        g.addColorStop(0.72, `rgba(155, 148, 136, ${op * 0.18})`);
        g.addColorStop(1,    'rgba(138, 130, 118, 0)');
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.arc(p.x, p.y, sz, 0, Math.PI * 2);
        ctx.fill();
      }

      smokeRafRef.current = requestAnimationFrame(smokeLoop);
    };

    smokeRafRef.current = requestAnimationFrame(smokeLoop);
    return () => {
      cancelAnimationFrame(smokeRafRef.current);
      ro.disconnect();
    };
  }, []);

  /* ── Render values ─────────────────────────────── */
  const ashH           = BODY_H * burnProgress;
  const remainingBodyH = BODY_H - ashH;
  const pct            = Math.floor(burnProgress * 100);

  return (
    <div
      ref={containerRef}
      className="relative flex flex-col items-center justify-center w-full h-full select-none overflow-hidden"
      style={{
        background: 'radial-gradient(ellipse at 50% 44%, #1c1308 0%, #060505 100%)',
        cursor: isDone ? 'default' : 'pointer',
      }}
      onTouchStart={(e) => { e.preventDefault(); startBurning(); }}
      onTouchEnd={stopBurning}
      onTouchCancel={stopBurning}
      onMouseDown={startBurning}
      onMouseUp={stopBurning}
      onMouseLeave={stopBurning}
    >
      {/* ── Full-area smoke canvas ──────────────── */}
      <canvas
        ref={canvasRef}
        className="absolute inset-0 pointer-events-none"
        style={{ zIndex: 5 }}
      />

      {/* ── HUD ────────────────────────────────── */}
      {!isDone && (
        <div
          className="absolute top-6 left-0 right-0 flex flex-col items-center gap-2 pointer-events-none"
          style={{ zIndex: 10 }}
        >
          <p
            className="text-sm transition-colors duration-500"
            style={{ color: isActive ? 'rgba(220,175,95,0.82)' : 'rgba(155,135,85,0.6)' }}
          >
            {isActive ? '🚬 흡연 중...' : '스페이스바를 꾹 누르세요 / 모바일: 화면을 꾹 터치'}
          </p>
          <div
            className="w-52 h-1.5 rounded-full overflow-hidden"
            style={{ background: '#1c1c1c' }}
          >
            <div
              className="h-full rounded-full"
              style={{
                width: `${pct}%`,
                background: 'linear-gradient(to right, #ff6a00, #ee0979)',
                transition: 'width 0.12s linear',
              }}
            />
          </div>
          <p className="text-xs" style={{ color: 'rgba(130,110,70,0.55)' }}>
            {pct}% 소비됨
          </p>
        </div>
      )}

      {/* ── Cigarette ──────────────────────────── */}
      <div
        style={{
          position: 'relative',
          width: CIGA_W,
          height: CIGA_TOTAL,
          overflow: 'visible',
          zIndex: 4,
          opacity: isDone ? 0.32 : 1,
          transition: 'opacity 0.6s',
        }}
      >

        {/* Ash */}
        {ashH > 1.5 && (
          <div
            style={{
              position: 'absolute',
              top: 0,
              left: '50%',
              transform: 'translateX(-50%)',
              width: CIGA_W - 5,
              height: ashH,
              background:
                'linear-gradient(to bottom, #d0d0c8 0%, #bbbbb2 30%, #a8a8a0 62%, #969690 100%)',
              borderRadius: '3px 3px 1px 1px',
            }}
          />
        )}

        {/* Ember — moves downward as burn progresses */}
        <div
          style={{
            position: 'absolute',
            top: ashH,
            left: 0,
            width: CIGA_W,
            height: EMBER_H,
            borderRadius: '2px',
            background: isActive
              ? 'radial-gradient(ellipse 88% 68% at 50% 38%, #ffee44 0%, #ffaa00 26%, #ff5500 54%, #991100 80%, transparent)'
              : 'radial-gradient(ellipse 72% 60% at 50% 52%, #ff8800 0%, #dd4400 46%, rgba(58,5,0,0.75) 100%)',
            boxShadow: isActive
              ? '0 0 10px 4px rgba(255,128,0,0.88), 0 0 24px 10px rgba(255,58,0,0.52), 0 0 48px 20px rgba(255,28,0,0.16)'
              : '0 0 5px 2px rgba(210,80,0,0.58)',
            animation: isActive ? 'flamePulse 0.2s ease-in-out infinite alternate' : undefined,
            zIndex: 3,
          }}
        />

        {/* Tobacco body — white cigarette paper */}
        {remainingBodyH > 0 && (
          <div
            style={{
              position: 'absolute',
              top: ashH + EMBER_H,
              left: 0,
              width: CIGA_W,
              height: remainingBodyH,
              background:
                'linear-gradient(to right, #c8c6c0 0%, #eeecea 26%, #f7f5f0 50%, #eeecea 74%, #c8c6c0 100%)',
              borderLeft: '1px solid #bcb8b0',
              borderRight: '1px solid #bcb8b0',
              overflow: 'hidden',
            }}
          >
            {/* Paper centre seam */}
            <div
              style={{
                position: 'absolute',
                left: '50%',
                top: 0, bottom: 0,
                width: 0.7,
                background: 'rgba(165,155,140,0.38)',
                transform: 'translateX(-50%)',
              }}
            />
            {/* Subtle tobacco show-through */}
            {[0.1, 0.24, 0.39, 0.54, 0.68, 0.82, 0.94].map((f, i) => (
              <div
                key={i}
                style={{
                  position: 'absolute',
                  top: `${f * 100}%`,
                  left: 3, right: 3,
                  height: 0.5,
                  background: 'rgba(148,132,108,0.2)',
                }}
              />
            ))}
          </div>
        )}

        {/* Gold foil band — FIXED position */}
        <div
          style={{
            position: 'absolute',
            top: BAND_TOP,
            left: 0,
            width: CIGA_W,
            height: BAND_H,
            background:
              'linear-gradient(to right, #8a7010 0%, #c8a012 18%, #e8c82a 35%, #f8e050 50%, #e8c82a 65%, #c8a012 82%, #8a7010 100%)',
          }}
        />

        {/* Cork filter — FIXED position */}
        <div
          style={{
            position: 'absolute',
            top: FILTER_TOP,
            left: 0,
            width: CIGA_W,
            height: FILTER_H,
            background:
              'linear-gradient(to right, #8A4A18 0%, #BE6E2C 18%, #D88848 35%, #E29058 50%, #D88848 65%, #BE6E2C 82%, #8A4A18 100%)',
            borderRadius: '0 0 5px 5px',
            overflow: 'hidden',
          }}
        >
          {/* Cork grain lines */}
          {Array.from({ length: 17 }, (_, i) => (
            <div
              key={i}
              style={{
                position: 'absolute',
                top: `${(i / 16) * 88 + 3}%`,
                left: 1, right: 1,
                height: 0.65,
                background: `rgba(${i % 2 === 0 ? '52,16,2' : '215,155,78'}, 0.22)`,
              }}
            />
          ))}
          {/* Bottom cap */}
          <div
            style={{
              position: 'absolute',
              bottom: 0,
              left: 0, right: 0,
              height: 7,
              background: 'rgba(65,22,4,0.28)',
              borderRadius: '0 0 5px 5px',
            }}
          />
        </div>

      </div>

      {/* ── Done overlay ───────────────────────── */}
      {isDone && (
        <div
          className="absolute inset-0 flex flex-col items-center justify-center gap-6"
          style={{ background: 'rgba(0,0,0,0.72)', zIndex: 20 }}
          onClick={e => e.stopPropagation()}
        >
          <p className="text-3xl font-bold" style={{ color: '#f0e5cc' }}>
            담배를 다 피웠습니다 💨
          </p>
          <p className="text-sm" style={{ color: 'rgba(165,145,105,0.82)' }}>
            건강을 위해 금연을 권장합니다
          </p>
          <button
            onClick={reset}
            className="mt-2 px-10 py-3 rounded-2xl text-sm font-semibold text-white"
            style={{ background: '#383838' }}
            onMouseOver={e => (e.currentTarget.style.background = '#505050')}
            onMouseOut={e =>  (e.currentTarget.style.background = '#383838')}
          >
            다시 피우기 (Reset)
          </button>
        </div>
      )}
    </div>
  );
}
